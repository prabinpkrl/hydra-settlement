...
export async function POST() {
  try {
    ...
    console.log("[/api/close] Sending close request via Alice's relay...");
    await hydraAlice.close(true);
    console.log("[/api/close] Close accepted.");

    return NextResponse.json({ ok: true });
  } catch (err: any) {
    const msg = err?.message ?? String(err);
    console.error("[/api/close] Error:", msg);
